...
io.on('connection', (socket) => {
    ...
    socket.on('signal', ({ target, targetRoom, type, sdp, candidate }) => {
        const payload = {
            sender: socket.username,
            type,
            sdp,
            candidate,
            roomContext: targetRoom // Keeps track if connection belongs to a specific group mesh
        };

        if (target) {
            const targetSocketId = usernameToIds[target];
            if (targetSocketId) {
